...
.fchip.sm { min-width: 0; padding: 6px 10px; }
.fchip.sm .s { font-size: 13.5px; }
.fchip.sm .m { font-size: 11px; }
.verdict { background: var(--surface); border: 1px solid var(--border);
  border-left: 4px solid var(--accent); border-radius: 10px;
  padding: 12px 16px; margin-bottom: 12px; }
.verdict .vh { font-size: 12px; color: var(--muted); margin-bottom: 3px;
  text-transform: none; letter-spacing: .02em; }
.verdict .vb { font-size: 15px; line-height: 1.75; }
...
function verdictBanner() {
  const c = DATA.backtest.cohorts;
  const all = c.all.h.m1, top = c.top.h.m1, tc = c.top_consensus.h.m1;
  if (!all.n) return "";
  const p0 = x => x===null||x===undefined ? "–" : (x*100).toFixed(0)+"%";
  let s = `照單全收 <b>${all.n}</b> 個部位只有 <b class="neg">${p0(all.beat)}</b> 贏大盤（平均超額 <b class="${cls(all.avg_ex)}">${pct(all.avg_ex)}</b>）`;
  if (top.n >= 10)
    s += `；改跟<b>前段班老師</b>升到 <b class="${top.beat>0.5?'pos':''}">${p0(top.beat)}</b>（超額 <b class="${cls(top.avg_ex)}">${pct(top.avg_ex)}</b>）`;
  if (tc.n >= 5)
    s += `，再要求<b>多位老師共識</b>可達 <b class="pos">${p0(tc.beat)}</b>（超額 <b class="${cls(tc.avg_ex)}">${pct(tc.avg_ex)}</b>）`;
  s += `。<b>結論：別照單全收，用老師戰績＋共識篩訊號。</b>`;
...
function focusCard() {
  ...
  const chip = (i, sm) => {
    const parts = [];
    if (i.n_teachers >= 2) parts.push(`<b>${i.n_teachers} 位</b>`);
  ...
  let html = `<div class="focus"><h2>📌 本週焦點</h2>
    <div class="win">${mdDate(f.start)}–${mdDate(f.end)} 共 ${f.n_episodes} 集 ｜ ★＝歷史超額為正的前段班老師</div>`;
  if (strong.length) {
    html += `<div class="subhd" style="color:var(--up)">🔥 強共識看多（3 位以上同看）</div><div class="fgrid">${strong.map(i => chip(i, false)).join("")}</div>`;
  }
  if (normal.length) {
    html += `<div class="subhd">看多（2 位同看）</div><div class="fgrid">${normal.map(i => chip(i, true)).join("")}</div>`;
  }
  if (f.shorts.length)
    html += `<div class="subhd" style="color:var(--down)">看空／建議避開</div><div class="fgrid">${f.shorts.map(i => chip(i, i.n_teachers < 3)).join("")}</div>`;
  html += `<div class="note" style="margin:10px 0 0">想看共識標的的歷史命中率驗證，點上方「共識標的」分頁。</div></div>`;
  return html;
}
